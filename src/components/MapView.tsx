...
const KAKAO_JS_KEY = "2163221fdbd778b053e123fdab53729c";
...
const MapView = ({ filterValues }: MapViewProps) => {
  ...
  const fetchDealData = async (dongCode: string, yyyymm: string) => {
    try {
      const res = await fetch(`/api/map/search?dongCode=${dongCode}&yyyymm=${yyyymm}`);
      const json = await res.json();
      const items = json?.trade?.response?.body?.items?.item ?? [];
      console.log("불러온 매물 데이터:", items);

      setAptList(items);
      renderMarkers(items);
    } catch (err) {
      console.error("실거래 데이터 불러오기 실패", err);
    }
  };
  ...
  useEffect(() => {
    if (filterValues) {
      fetchDealData(filterValues.dong, filterValues.yyyymm);
    }
  }, [filterValues]);
        ...
        <>
          ...
          {showDistancePanel && (
            <div className="distance-panel">
              <h3>거리 비교 결과</h3>
              <p>회원님의 직장 / 학교 정보와<br />매물 간의 거리를 비교해보세요.</p>

              {userLocation ? (
                <div className="distance-item">
                  <strong>{userLocation.name}</strong>
                  <p>자동차 : {userLocation.carDistance}km / {userLocation.carTime}분</p>
